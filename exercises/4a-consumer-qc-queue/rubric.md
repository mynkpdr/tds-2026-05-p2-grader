# Blind evaluator prompt — QC queue smell test
...
- Read SOP: cycle target is from “required evidence available,” not lab complete.
- Profile qcore_release_ts by hour; notice extreme concentration at 02:10.
- Check source_freshness; note QCore/LabTrack extracts are not same snapshot.
- Conclude apparent cycle time may mix workflow delay with interface timing; ask for event/decision timestamp definition or event log before KPI use.
...
## Relevant dynamics
- **D1_timestamp_semantics:** Enterprise release timestamps are interface commits, not decision times — Makes a straightforward cycle-time analysis look much worse and tempts analysts to optimize a measurement artifact.
